...
resource "azurerm_local_network_gateway" "ptp_vpn_local_gw" {
  ...
  resource_group_name = azurerm_resource_group.network_rg.name
  location            = var.region
  gateway_fqdn        = var.ptp_vpn_remote_endpoint
...
resource "azurerm_public_ip" "virtual_network_gateway_public_ip" {
  name                = "${var.environment_tag}-${var.region}-VGW-IP"
  location            = var.region
  resource_group_name = azurerm_resource_group.network_rg.name
...
resource "azurerm_virtual_network_gateway" "virtual_network_gateway" {
  ...
  name                = "${var.environment_tag}-${var.region}-VPN-GW"
  location            = var.region
  resource_group_name = azurerm_resource_group.network_rg.name
  type                = "Vpn"
  vpn_type            = "RouteBased"
...
resource "azurerm_virtual_network_gateway_connection" "onpremise" {
  ...
  name                       = "${var.environment_tag}-${var.region}-VPN-On-Prem"
  location                   = var.region
  resource_group_name        = azurerm_resource_group.network_rg.name
  type                       = "IPsec"
  virtual_network_gateway_id = azurerm_virtual_network_gateway.virtual_network_gateway.id
  local_network_gateway_id   = azurerm_local_network_gateway.ptp_vpn_local_gw.id
